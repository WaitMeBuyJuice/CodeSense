...
CodeSense 服务启动时通过 **MCP Prompts 协议**自动注册两个内置 Skill，宿主 Agent（如 CodeMaker）连接后即可通过 `prompts/list` 发现、`prompts/get` 获取，无需手动安装：
...
Skill 文件随 Python 包分发（`src/codesense_v1/skills/`），版本与服务始终对齐。
...
├── skills/        # 内置 Skill 文件（通过 MCP Prompts 自动注入）
