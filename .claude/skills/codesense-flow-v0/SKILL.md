...
description: 代码架构探索工作流。适用场景：探索代码库、理解某模块结构、询问某模块的作用或策略、定位某个功能归属模块、在理解某模块后对其进行修改。不适用场景：用户已明确给出文件路径、行号或具体符号名。激活后引导按"全局架构 → 模块接口 → 代码细节"三步走。
---
...
The `project_map` resource from the CodeSense MCP server provides a project-wide overview.
Read it via the MCP `resources/read` mechanism to understand:
...
name as listed in `project_map` (e.g. `缓存层`, `Data Layer`).
...
输出：
1. 读取 project_map → 确认 cache 模块位置与依赖、它被哪些模块引用
2. explore_module("缓存层") → 看公开接口和内部文件
...
- `explore_module` 的 `module_name` 是 project_map 中列出的模块名（如 `缓存层`），由 LLM 在生成 project_map 时确定。如不知道有哪些模块，先读取 `codesense://project_map`。
