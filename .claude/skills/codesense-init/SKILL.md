...
2. 调用 `submit_project_map(response=<模块划分文本>)`
...
3. 若返回"尚未生成摘要"（cache miss）：
   ...
   - 按提示词生成模块 Markdown 摘要，包含：
     ...
     - 上下游关系
     - 实现约束清单
   - 调用 `save_module_summary(module_name=<模块名>, summary=<生成的摘要>)`
...
对 Phase 2 中处理过的每个模块，逐文件生成子模块文档。

**判断是否需要 Phase 3**：
- 单文件模块（除 `__init__.py` 外只有 0-1 个 .py 文件）跳过，`explore_submodule` 会返回引导跳过的提示
- 多文件模块中的 `__init__.py` 跳过

**每个文件的处理步骤**：

1. 调用 `explore_submodule(module_name=<模块名>, file_path=<文件相对路径>)`
2. 若返回缓存命中的文档 → 跳过
3. 若返回"尚未生成文档"（cache miss）：
   - 工具返回体中包含完整分析提示词
   - 按提示词生成文件 Markdown 文档，包含：
     - 文件概述
     - 对外接口（函数/类签名）
     - 跨模块依赖
     - 典型调用链
   - 调用 `save_submodule_summary(module_name=<模块名>, file_path=<文件路径>, summary=<生成的文档>)`
...
| Phase 3 | 所有子模块文档生成 | 每个 `explore_submodule` 均命中缓存或返回跳过提示 |
