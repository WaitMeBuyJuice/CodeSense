"""MCP Tool: get_module_prompt — returns the LLM prompt for a specific module summary."""
...
@tool(
    name="get_module_prompt",
    description=(
        "返回用于生成指定模块摘要的分析提示词文本。\n\n"
        "仅在 explore_module 返回生成步骤引导时使用，通常委派给子 Agent 执行。\n"
        "正常使用时无需主动调用本工具。\n\n"
        "module_name 必须是 project_map 返回的模块名之一。\n"
        "获取提示词后，生成 Markdown 格式的模块摘要，"
        "再调用 save_module_summary 保存结果。"
    ),
    input_schema=_SCHEMA,
)
async def get_module_prompt_tool(module_name: str) -> str:
    ...
    try:
        return await summarizer.get_module_prompt(Path(project_root_str), module_name)
