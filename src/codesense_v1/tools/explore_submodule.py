...
async def explore_submodule(
    ...
    if subgroup_name is not None:
        ...
        return (
            f"# 子模块「{subgroup_name}」尚未生成文档，必须按照以下步骤生成文档\n\n"
            ...
            "> 3. 完成后回复\"已完成\"\n\n"
            f"子 Agent 完成后，主 Agent 重新调用 `explore_submodule(module_name=\"{name}\", subgroup_name=\"{subgroup_name}\")` 即可获取最终文档。\n\n"
            "## 方式 2：主 Agent 直接执行（适用于无子 Agent 能力的场景）\n\n"
            "1. 阅读下方「分析提示词」，生成该子模块的 Markdown 文档（包含：子模块概述、对外能力、跨模块依赖、典型调用链）\n"
            f"2. 调用 `save_submodule_summary(module_name=\"{name}\", subgroup_name=\"{subgroup_name}\", summary=<生成的文档>)` 保存\n"
            f"3. 重新调用 `explore_submodule(module_name=\"{name}\", subgroup_name=\"{subgroup_name}\")` 获取结果\n\n"
            "---\n\n"
            "## 分析提示词\n\n"
            f"{prompt_text}\n"
        )
    ...
    return (
        ...
        f"2. 调用 `save_submodule_summary(module_name=\"{name}\", file_path=\"{file_path}\", summary=<生成的文档>)` 保存\n"
        f"3. 重新调用 `explore_submodule(module_name=\"{name}\", file_path=\"{file_path}\")` 获取结果\n\n"
        "---\n\n"
        "## 分析提示词\n\n"
        f"{prompt_text}\n"
    )
