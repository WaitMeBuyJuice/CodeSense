"""MCP Tool: get_flows_segment_prompt — returns LLM prompt for key flow descriptions."""
...
from codesense_v1.summarizer import get_flows_segment_prompt
...
@tool(
    name="get_flows_segment_prompt",
    description=(
        "返回用于生成关键流程描述段（05_flows）的 LLM 分析提示词。\n\n"
        "仅在 project_map 提示「05_flows 需要生成」时调用。\n"
        "需要先完成 03_modules 的生成。\n"
        "获取提示词后，按格式生成内容，再调用 save_project_map_segment 保存。"
    ),
    input_schema=_SCHEMA,
)
async def get_flows_segment_prompt_tool() -> str:
    ...
    try:
        return await get_flows_segment_prompt(project_root)
