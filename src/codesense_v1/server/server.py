...
2. **写回缓存**：调对应 save 工具保存生成内容：
   - `project_map` 各段 → `save_project_map_segment(segment_id=..., content=...)`（`03_modules` 通过 `submit_project_map` 提交，无需单独 save_project_map_segment）
...
4. 遇到非预期结果（保存成功但重调仍显示 cache miss、hash 不匹配等）立即停止并向用户说明，不要自行读源码排查
