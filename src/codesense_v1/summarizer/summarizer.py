...
def _build_module_prompt(
    ...
    *,
    ...
    return (
        ...
        "## 子模块列表\n"
        "（表格，3 列：子模块名 | 职责 | 包含文件；\n\n"
        "  **硬性划分规则**：详见 SERVER_INSTRUCTIONS § subgroups 约束；此处补充：若已有 subgroups 定义则必须严格按原样复制；\n\n"
        ...
        "## 实现约束清单\n"
        "（边界行为、踩坑点、禁忌；**只写模块内部约束**；此处仅输出本模块内部约束；如确有跨模块强制性规则，需在摘要中明确标注原因）\n\n"
        "## subgroups（JSON）\n"
...
def _build_submodule_prompt(
    ...
    *,
    ...
    lines += [
        ...
        "  接口模块：以表格列出 API 端点/MCP 工具名/事件，格式自行选择（一般目录名或文件名包含 api / endpoint / server / handler 等关键词时为接口模块；否则按照涉及 I/O、外部协议的能力判断）；",
        "  完全内部模块：写「仅供模块内部调用」）",
        "",
        "## 跨模块依赖",
